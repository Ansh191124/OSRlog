...
export default function Approvals() {
  ...
  const columns = [
    ...
    { key: 'details', header: 'Details', render: r => <span className="text-xs text-steel max-w-xs block truncate" title={r.details}>{r.details || '—'}</span> },
    ...
    { key: 'paymentType', header: 'Payment type', render: r => <Badge tone="steel">{r.paymentType || 'cash'}</Badge> },
  ...
  return <div><PageHeader eyebrow="Controlled spending" title="Approval queue" description="Every request clearly shows approval status and payment type: Request → Approval → Accountant payment." action={<button onClick={() => setOpen(true)} className="btn-accent px-3 py-2 rounded text-sm">New request</button>} />
